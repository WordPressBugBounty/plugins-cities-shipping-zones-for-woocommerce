...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || get_site_option( 'active_sitewide_plugins') && array_key_exists( 'woocommerce/woocommerce.php', get_site_option( 'active_sitewide_plugins' ) ) ) {
	...
	class WC_CSZ {
			...
			if ( ! empty( $value ) && ! empty( $_POST['wc_csz_countries_codes'] ) && ! empty( $_POST['wc_csz_set_zone_country'] ) && ! empty( $_POST['wc_csz_set_zone_id'] ) ) {
				...
				if ( $selected_country_codes && in_array( $_POST['wc_csz_set_zone_country'], $selected_country_codes ) ) {
					...
					if ( ! empty( $locations_codes ) ) {
						...
						if ( apply_filters( 'csz_set_locations_keep_old_locations_enabled', false ) ) { // can also add option to remove locations instead of adding them
		...
		/**
		 * Copy from the state field to the city field on account pages
		 * @param mixed $user_id
		...
		public function wc_customer_copy_state_city( $user_id, $load_address ) {
			if ( apply_filters( 'csz_customer_account_city_update_enabled', true ) ) {
				...
				if ( in_array( $customer_country_code, get_option( 'wc_csz_countries_codes' ) ) ) { // Need also to make sure the country and state fields are not empty, can aslo add notice when happen (on admin)
		...
		/**
		 * Prevent the city of the store from beeing the default customer city (when store location configured as default)
